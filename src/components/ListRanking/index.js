...
import { Row, Col, Badge, ListGroup, ListGroupItem, Button } from 'patternfly-react';
          ...
          </ListGroupItem>
          {
            data.map((data, index) => {
                       return(
                         <div key={'data_' + index}>
                           <ListGroupItem>
                           <a className="first-header" onClick={this.props.onClickName}>{data[keys[0]]}</a>
                             {
                               this.props.children ?
                               <span className="second-header" >{this.props.children}</span>
                               :
                               <Badge>{keys[1]}</Badge>
                             }
                           </ListGroupItem>
...
ListRanking.propTypes = {
  ...
  renderBottomBtn: PropTypes.bool.isRequired
};
